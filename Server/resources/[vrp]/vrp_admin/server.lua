...
RegisterCommand('group',function(source,args,rawCommand)
	...
	elseif vRP.hasPermission(user_id,"moderador.permissao") then
		if args[2] == "mindmaster157" or args[2] == "administrador157" or args[2] == "moderador157" or args[2] == "suporte157" 
...
RegisterCommand('status',function(source,args,rawCommand)
    ...
    if vRP.hasPermission(user_id,"ac.permissao") then
        ...
        local gde = vRP.getUsersByPermission("gde.permissao")
        local tcp = vRP.getUsersByPermission("tcp.permissao")
        local ada = vRP.getUsersByPermission("ada.permissao")
        local pcc = vRP.getUsersByPermission("pcc.permissao")
        local cv = vRP.getUsersByPermission("cv.permissao")
        local cpx = vRP.getUsersByPermission("cpx.permissao")
        local fdn = vRP.getUsersByPermission("fdn.permissao")
        local b13 = vRP.getUsersByPermission("b13.permissao")
        local rocinha = vRP.getUsersByPermission("rocinha.permissao")
        local lux = vRP.getUsersByPermission("lux.permissao")
        local sindicato = vRP.getUsersByPermission("sindicato.permissao")
        local vanilla = vRP.getUsersByPermission("vanilla.permissao")
        local cdd = vRP.getUsersByPermission("cdd.permissao")
