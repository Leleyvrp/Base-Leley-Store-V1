...
function vRP.getUserIdByIdentifiers(ids)
	if ids and #ids then
		...
		local rows = exports["oxmysql"]:executeSync([[INSERT INTO vrp_users(whitelisted,banned) VALUES(true,false)]])
		if rows then
...
vRP.prepare("vRP/create_user","INSERT INTO vrp_users(whitelisted,banned) VALUES(true,false)")
			...
			-- if not check then
				...
				if not vRP.isBanned(user_id) then
					deferrals.update("Carregando whitelist.")
						...
						deferrals.done("Mande seu ID nosso Discord! [ discord.gg/6QhQQgp7dG ][ ID: "..user_id.." ]")
						TriggerEvent("queue:playerConnectingRemoveQueues",ids)
					end
				else
					deferrals.done("Você foi banido! [ Compre seu unban: discord.gg/6QhQQgp7dG ] [ ID: "..user_id.." ] ")
					--print(user_id)
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
		else
