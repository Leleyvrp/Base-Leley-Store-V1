...
function valueToNui()
    data[1] = cFG.nomeBanco
    data[2] = cFG.imgBanco
    -- if not cache then
    --     cache = true
    -- end

    info = NexusServer.getTransacoes()

    pix = NexusServer.Pixcadastrado()
    data[16] = 0
    -- if not cachePix then
    data[5], data[2], data[11], data[12], data[15] = NexusServer.getIdentity()
    --     cachePix = true
    -- end
    -- if not cacheGraph then
   

    --     cacheGraph = true
    -- end
   -- print(json.encode(data))
    SendNUIMessage({action = 'setInformation', data = data, info = info, pix = pix})
    SendReactMessage('setDashboardInformation', data)
end
