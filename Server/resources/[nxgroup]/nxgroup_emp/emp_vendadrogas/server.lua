...
local drogasPrice = {
    {"maconha",1200,1500,1,5,"tcp.permissao"},
...
function eG.receberVendaDroga(bonus)
	...
	if user_id then
		if vRP.getInventoryItemAmount(user_id,'maconha') <= 0 and vRP.getInventoryItemAmount(user_id,'cocaina') <= 0 and vRP.getInventoryItemAmount(user_id,'metanfetamina') <= 0 and vRP.getInventoryItemAmount(user_id, 'lancaperfume') <= 0 and vRP.getInventoryItemAmount(user_id, 'lsd') <= 0 then
